...
describe("TestMarket 5006", function () {
    ...
    beforeEach(async function () {
        ...
        await testERC5006.mint(ownerOfNFT.address, secondTokenId, 100)

        await testERC5006.connect(ownerOfNFT).setApprovalForAll(bank.address, true);
        ...
        nft = { tokenType: TokenType.ERC5006, token: testERC5006.address, tokenId: firstTokenId, amount: 100 }
        rentalPrice_ETH = { paymentToken: ethers.constants.AddressZero, pricePerCycle: ethers.utils.parseEther('1'), cycle: 86400 }
        //metadata
        let tokenURI = await testERC5006.uri(firstTokenId);
    ...
    describe('ERC5006-fulfillLendOrder1155', function () {
        ...
        it("fulfillLendOrder1155 success", async function () {
            ...
            let order_hash_local = typedDataEncoder_lendOrder.hashStruct('LendOrder', lendOrder);
            await receipt.wait();
            let record = await testERC5006.userRecordOf(1);
            await expect(receipt).to.emit(market, "LendOrderFulfilled")
            await expect(receipt).to.emit(testERC5006, "CreateUserRecord").withArgs(1, firstTokenId, 10, bank.address, renterA.address, record[4]);
    ...
    describe('ERC5006-fulfillRentOffer1155', function () {
        let rentOffer_A: RentOffer;
        let rentOffer_B: RentOffer;
        let rentalPrice_ERC20;
        beforeEach(async function () {
            let nft_rent = { tokenType: TokenType.ERC5006, token: testERC5006.address, tokenId: firstTokenId, amount: 10 }
            rentalPrice_ERC20 = { paymentToken: erc20.address, pricePerCycle: ethers.utils.parseEther('1'), cycle: 86400 }
            rentOffer_A = {
        ...
        it("fulfillRentOffer1155 success", async function () {
            ...
            let record_a = await testERC5006.userRecordOf(1);
