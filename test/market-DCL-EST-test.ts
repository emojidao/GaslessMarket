...
describe("TestMarket 721 DCL", function () {
    ...
    beforeEach(async function () {
        ...
        const MetadataCheckerEST = await ethers.getContractFactory("MetadataCheckerEST");
        metadataChecker721 = await MetadataCheckerEST.deploy();
        ...
        const EstateRegistry = await ethers.getContractFactory("contracts/test/dcl/EstateRegistry.sol:EstateRegistry");
        testERC721 = await EstateRegistry.deploy();
        

        const LANDRegistry = await ethers.getContractFactory("contracts/test/dcl/LANDRegistry.sol:LANDRegistry");
        testLand = await LANDRegistry.deploy();
        await testLand.setEstateRegistry(testERC721.address);
        await testERC721["initialize(string,string,address)"]("EST","EST",testLand.address);
        ...
        // firstTokenId = await testLand._unsafeEncodeTokenId(100, 100);
        // secondTokenId = await testLand._unsafeEncodeTokenId(100, 101);

        await testLand.updateWhiteListMap([ownerOfMarket.address], true);
        await testLand.assignMultipleParcels([100,100,100,100], [100,101,102,103], ownerOfNFT.address)

        await testLand.connect(ownerOfNFT).createEstate([100, 100], [100, 101], ownerOfNFT.address);
        await testLand.connect(ownerOfNFT).createEstate([100, 100], [102, 103], ownerOfNFT.address);
        firstTokenId = await testERC721.tokenOfOwnerByIndex(ownerOfNFT.address,0);
        secondTokenId = await testERC721.tokenOfOwnerByIndex(ownerOfNFT.address,1);
        ...
        //metadata
        let fingerprint = await testERC721.getFingerprint(firstTokenId);
        let metadataHash_local = ethers.utils.solidityKeccak256(['bytes'], [fingerprint]);
